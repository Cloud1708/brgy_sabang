<?php
// bns_modules/api_feeding_programs.php
...
switch ($method) {
    case 'GET':
        // List all upcoming feeding program events
        $sql = "SELECT * FROM events WHERE event_type = 'feeding' AND event_date >= CURDATE() ORDER BY event_date, event_time";
    ...
    case 'POST':
        ...
        if (isset($data['event_id']) && $data['event_id']) {
            // Update
            $stmt = $mysqli->prepare("UPDATE events SET event_title=?, event_description=?, event_date=?, event_time=?, location=?, target_audience=?, is_published=?, updated_at=NOW() WHERE event_id=? AND event_type='feeding'");
    ...
    case 'DELETE':
        // Delete event
        $data = json_decode(file_get_contents('php://input'), true);
        $stmt = $mysqli->prepare("DELETE FROM events WHERE event_id=? AND event_type='feeding'");
