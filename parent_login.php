...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['username'])) {
    ...
    if ($lock_status['locked']) {
        ...
        if ($remaining_attempts < 5) {
            $msg = "<div class='alert alert-warning py-2 small mb-3'><i class='bi bi-exclamation-triangle me-1'></i>{$remaining_attempts} attempt(s) remaining before account lockout.</div>";
        }
    }
} else {
    if ($err === '1') {
  ...
  <main class="container d-flex align-items-center justify-content-center auth-stage">
    <div class="card login-card shadow-lg border-0">
      <div class="card-body p-4 p-md-5">
        ...
        <form method="post" action="auth/login_parent_process" novalidate <?php echo $is_locked ? 'onsubmit="return false;"' : ''; ?>>
          <div class="mb-3">
